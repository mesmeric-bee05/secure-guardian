...
CREATE POLICY "Admins can manage all cases" ON public.emergency_cases AS PERMISSIVE FOR ALL TO authenticated USING (public.is_admin(auth.uid())) WITH CHECK (public.is_admin(auth.uid()));
CREATE POLICY "CHWs can view assigned cases" ON public.emergency_cases AS PERMISSIVE FOR SELECT TO authenticated USING (public.is_chw(auth.uid()) AND (assigned_chw_id = auth.uid() OR assigned_chw_id IS NULL));
CREATE POLICY "CHWs can update assigned cases" ON public.emergency_cases AS PERMISSIVE FOR UPDATE TO authenticated USING (public.is_chw(auth.uid()) AND assigned_chw_id = auth.uid());
...
CREATE POLICY "Admins can view all profiles" ON public.profiles AS PERMISSIVE FOR SELECT TO authenticated USING (public.is_admin(auth.uid()));
CREATE POLICY "CHWs can view assigned user profiles" ON public.profiles AS PERMISSIVE FOR SELECT TO authenticated USING (public.is_chw(auth.uid()) AND EXISTS (SELECT 1 FROM public.emergency_cases ec WHERE ec.user_id = profiles.user_id AND ec.assigned_chw_id = auth.uid()));
...
CREATE POLICY "Admins can manage all roles" ON public.user_roles AS PERMISSIVE FOR ALL TO authenticated USING (public.is_admin(auth.uid())) WITH CHECK (public.is_admin(auth.uid()));
...
CREATE POLICY "Admins can manage assignments" ON public.chw_assignments AS PERMISSIVE FOR ALL TO authenticated USING (public.is_admin(auth.uid())) WITH CHECK (public.is_admin(auth.uid()));
...
CREATE POLICY "Users can manage messages in their sessions" ON public.chat_messages AS PERMISSIVE FOR ALL TO authenticated USING (EXISTS (SELECT 1 FROM public.chat_sessions cs WHERE cs.id = chat_messages.session_id AND cs.user_id = auth.uid()));
...
CREATE POLICY "Admins can view all SMS logs" ON public.sms_logs AS PERMISSIVE FOR SELECT TO authenticated USING (public.is_admin(auth.uid()));

-- audit_logs
DROP POLICY IF EXISTS "Admins can view audit logs" ON public.audit_logs;
DROP POLICY IF EXISTS "Authenticated users can insert audit logs" ON public.audit_logs;
CREATE POLICY "Admins can view audit logs" ON public.audit_logs AS PERMISSIVE FOR SELECT TO authenticated USING (public.is_admin(auth.uid()));
...
CREATE POLICY "Admins can manage facilities" ON public.health_facilities AS PERMISSIVE FOR ALL TO authenticated USING (public.is_admin(auth.uid())) WITH CHECK (public.is_admin(auth.uid()));
...
CREATE POLICY "Admins can manage protocols" ON public.first_aid_protocols AS PERMISSIVE FOR ALL TO authenticated USING (public.is_admin(auth.uid())) WITH CHECK (public.is_admin(auth.uid()));

-- Restore the on_auth_user_created trigger
