...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader?.startsWith("Bearer ")) {
      logSecurityEvent({ event_type: "auth_failed", scope: "reports-export", ip_address: ip, severity: "warn", details: { reason: "missing_bearer" } });
    ...
    if (claimsError || !claims?.claims) {
      logSecurityEvent({ event_type: "auth_failed", scope: "reports-export", ip_address: ip, severity: "warn", details: { reason: "invalid_token" } });
      return new Response(JSON.stringify({ error: "Unauthorized" }), {
        status: 401, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
